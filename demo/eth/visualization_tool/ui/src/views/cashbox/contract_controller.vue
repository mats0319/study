    ...
    <div class="ed-constructor-params">
      <el-form v-model="params" ref="form" label-width="15rem" status-icon>
        ...
        <el-form-item label="D合约地址">
          <el-input v-model="params.DContractAddr" placeholder="0x..." @change="validDContractAddr" />
...
import { controllerContractByteCode, libraries } from "@/views/cashbox/data"
...
export default class CashboxControllerDeployC extends Vue {
  ...
  // const, for html
  private contractByteCode = controllerContractByteCode;
  ...
  private deploy(): void {
    ...
    let byteCode = controllerContractByteCode;
    for (let i = 0; i < libraries.length; i++) {
    ...
    //@ts-ignore
    this.$refs.sendTx.sendTx(tx, 0, "deploy cashbox C contract");
  }
