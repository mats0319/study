<template>
  <div class="eth-deploy">
    <div class="ed-info-item">
      <label>链地址&#58;&nbsp;</label>
      <el-input
        v-model="$store.state.ethChainAddress"
        placeholder="https://goerli.infura.io/v3/63ed578c193242289ab781596516987a"
      />
    </div>
...
export default class ETHContract extends Vue {
  private params = {
    admin1: "",
    admin2: "",
    admin3: "",
  ...
  private deploy(): void {
    ...
    //@ts-ignore
    this.$refs.sendTx.sendTx(tx, 0, "deploy eth contract");
  }
  ...
  private getAddress(): void {
    ...
    getTxReceipt(this.$store.state.ethChainAddress, this.deployTxHash, (receipt: TransactionReceipt) => {
      this.$message.success("获取合约地址成功：" + receipt.contractAddress);
      this.$store.state.ethContractAddress = receipt.contractAddress as string;
    });
  }
