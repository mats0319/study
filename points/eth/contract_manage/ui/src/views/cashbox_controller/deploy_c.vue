  ...
  <div class="eth-deploy">
    ...
    <div class="ed-constructor-params">
      ...
      >
        ...
        <el-form-item label="NFT官方持有者">
          <el-input v-model="params.nftHolder" placeholder="0x..." @change="validNFTHolder" />
          <i v-show="validParams.nftHolder === 0" class="el-icon-loading gray" />
          <i v-show="validParams.nftHolder > 0" class="el-icon-check green" />
          <i v-show="validParams.nftHolder < 0" class="el-icon-close red" />
        </el-form-item>

        <el-form-item label="D合约地址">
          <el-input v-model="params.DContractAddr" placeholder="0x..." @change="validDContractAddr" disabled />
          <i v-show="validParams.DContractAddr === 0" class="el-icon-loading gray" />
          <i v-show="validParams.DContractAddr > 0" class="el-icon-check green" />
          <i v-show="validParams.DContractAddr < 0" class="el-icon-close red" />
        </el-form-item>
      </el-form>
    </div>
...
import { cashboxDataContractAddress, chainAddr, contractByteCode, libraries } from "./data"
import { TransactionReceipt } from "web3-core";
import { ElLoadingComponent } from "element-ui/types/loading";

@Component
export default class CashboxControllerDeployC extends Vue {
  private params = {
    admin1: "",
    admin2: "",
    admin3: "",
    decisionMaker: "",
    nftHolder: "",
    DContractAddr: this.$store.state.cashboxDContractAddress,
  }
  ...
  private mounted() {
    if (this.$store.state.cashboxDContractAddress.length < 1) {
      this.$store.state.cashboxDContractAddress = cashboxDataContractAddress;
      this.params.DContractAddr = cashboxDataContractAddress;
    }

    this.validDContractAddr(this.params.DContractAddr);
  }
  ...
  private deploy(): void {
    ...
    if (this.validParams.admin1 <= 0 ||
      ...
      this.validParams.nftHolder <= 0 ||
      this.validParams.DContractAddr <= 0
    ) {
      this.$message.info("当前输入参数有误，请检查后重试");
      return;
    }

    const params = encodeParameters([ "address[3]", "address", "address", "address" ], [
      [ this.params.admin1, this.params.admin2, this.params.admin3 ],
      this.params.decisionMaker,
      this.params.nftHolder,
      this.$store.state.cashboxDContractAddress,
    ])
    ...
    sendTransaction(tx)
      .then((txHash: any) => {
        ...
        console.log("> deploy cashbox C contract success, tx hash: ", txHash);
      })
      .catch((err: any) => {
        this.$message.error(err);
        console.log("> deploy cashbox C contract failed, error: ", err);
      })
      .finally(() => {
  ...
  private validNFTHolder(value: string): void {
    this.validParams.nftHolder = isValidAddress(value) ? 1 : -1;
  }

  private validDContractAddr(value: string): void {
    this.validParams.DContractAddr = isValidAddress(value) ? 1 : -1;
  }
}
</script>
