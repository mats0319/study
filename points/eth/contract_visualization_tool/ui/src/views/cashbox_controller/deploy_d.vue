<template>
  <div class="eth-deploy">
    <div class="ed-info-item">
      <label>链地址&#58;&nbsp;</label>
      <el-input v-model="$store.state.cashboxChainAddress" placeholder="http://..." />
    </div>
    ...
    <div class="ed-byte-code">
      ...
      <el-input v-model="cashboxDataContractByteCode" type="textarea" :rows="10" resize="none" readonly />
    </div>
  </div>
</template>
...
import { cashboxDataContractByteCode, libraries } from "./data"
...
export default class CashboxControllerDeployD extends Vue {
  ...
  // const, for html
  private cashboxDataContractByteCode = cashboxDataContractByteCode;
  ...
  private deploy(): void {
    ...
    let byteCode = cashboxDataContractByteCode;
    for (let i = 0; i < libraries.length; i++) {
    ...
    sendTransaction(tx)
      .then((txHash: any) => {
        ...
        console.log("> deploy cashbox D contract success, tx hash: ", txHash);

        this.deployTxHash = txHash;
      })
      .catch((err: any) => {
        this.$message.error(err);
        console.log("> deploy cashbox D contract failed, error: ", err);
      })
      .finally(() => {
